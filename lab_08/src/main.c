...
int main(void)
{
    ...
    while (1)
    {
        ...
        {
            puts("Введите количество вершин, затем матрицу: ");
        ...
        {
            puts("Введите количество вершин, затем ребра в виде 'V1 вес V2'");
            puts("Для завершения ввода введите букву");
            if (list_graph_read(stdin, list_graph))
            {
                puts("При вводе возникла ошибка");
                continue;
            }
            puts("Список успешно прочитан\n");
        }
        else if (menu == 7)
        {
            if (list_graph->len == 0)
